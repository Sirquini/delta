...
class ProgressRange():
    """A class wrapper for iterators that prints the progress status.
    
    Behaves like an iterator, as such may be used in a for loop like one
    ...
    for i in ProgressRange.from_iter(even_numbers):
        result.append(i)
        time.sleep(1)
    ...
    @classmethod
    def from_iter(cls, it, stop=0, status=''):
        '''Creates a new iterable wrapping `it`.

        Yields elements generated by calling `next(it)`.
        Optionaly, displays a status message during iteration.

        If `stop = 0`, tries to use `len(it)` when available. 
        
        Args:
            it (int): The iterator to wrap and yield values from.
            stop (int, optional): An estimated upper limit.
            status (str, optional): A message next to the progress bar.
        '''
    ...
    def __next__(self):
        try:
            ...
            raise stop
        progress_bar(self.counter, self.max, status=self.status)
        self.counter += 1
